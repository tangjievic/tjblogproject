...
import { Row, Col, Button, Tag, Divider, Pagination,message } from 'antd';
...
class QueryArt extends Component{
    constructor(props){
        super(props)
        this.state = {
            select_current: 1,
    ...
    render(){
        //console.log(this.state.keywrods)
        return(
            <div className="queryart_box">
                {/* <h2>查找结果：</h2> */}
                <Row>
                    <Col span={12}>
                    <h2>查找结果：</h2>
                    ...
                    <Col span={12} style={{textAlign:'right'}}>
                        <Button className="btn" type="link" onClick={()=>this.goBack()}>返回至主页</Button>
                    </Col>
                </Row>

            {
                (() => {
                    if (!this.state.artlist || (this.state.artlist ? this.state.artlist.length === 0 : true)) {
                        return (
            ...
            current={this.state.select_current}
    ...
    static getDerivedStateFromProps(nextProps, prevState){
        if(nextProps.match.params.keywrods !== prevState.keywrods){
            return {
                keywrods: nextProps.match.params.keywrods,
            };
        }
        return null
    }
    ...
    componentDidMount(){
        this.getQueryKeywordsArt({
            page_count:1,
            page_size:10,
            type:5,
            keywrods:this.props.match.params.keywrods
        })
    }

    componentDidUpdate(prevState){
        console.log(prevState.match.params.keywrods,this.state.keywrods)
        if(this.state.keywrods!==prevState.match.params.keywrods ){
            this.getQueryKeywordsArt({
                page_count:1,
                page_size:10,
                type:5,
                keywrods:this.state.keywrods
            })
        }
    }

    //get查询文章列表
    getQueryKeywordsArt(params = {
        page_count:1,
        page_size:10,
        type:5,
        keywrods:''
        ...
        getArtList({
            ...
            keywrods: params.keywrods
        }).then(res=>{
            this.setState({
                artlist:res.data.list.data,
                page_total:res.data.pagenation.total
            })
        })
    }
    getCateName(cate_id) {
...
export default connect(mapState)(QueryArt)